...
model.data = model.data %>% 
  mutate(churn = factor(trainLabels$churn))
model.data$churn = factor((ifelse(model.data$churn == 1, "P", "N")))

# Split Data - Only for development
#model.data.training <-
# createDataPartition(model.data$churn, p=0.75, list=FALSE)
...
#  data = model.data.train  
  ...
  set.seed(2017)
  data.train = data %>% select(-c(churn))
  gbm.tune <- train(x=data.train, y=data$churn,
                  method = "gbm",
                  metric = "ROC",
                  trControl = ctrl,
                  tuneGrid=grid,
