...
model.data = model.data %>% 
  mutate(upselling = factor(trainLabels$upselling))
model.data$upselling = factor((ifelse(model.data$upselling == 1, "P", "N")))

# Split Data - Only for development
#model.data.training <-
# createDataPartition(model.data$upselling, p=0.75, list=FALSE)
...
#  data = model.data.train  
  ...
  set.seed(2017)
  data.train = data %>% select(-c(upselling))
  gbm.tune <- train(x=data.train, y=data$upselling,
                  method = "gbm",
                  metric = "ROC",
                  trControl = ctrl,
                  tuneGrid=grid,
...
boosting.predict <- function(fitted.model, data) {
   ...
   data <- apply_pre_processing(data, model.preProcessingFunctions)
   data.test = data %>% select(-c(upselling))
   model.pred = predict(fitted.model, data.test)
}

model.table <- table(
  "Prediction" = model.pred,
  "Truth" = model.data.test$upselling
)
