...
#' function - should be used to score all of our AUCs. 
...
calculate_metrics = function(table, debug=FALSE) {
  print (table)
  tn <- if (table[1] == 0) 0 else table[1] 
  fp <- if (table[2] == 0) 0 else table[2]
  fn <- if (table[3] == 0) 0 else table[3]
  tp <- if (table[4] == 0) 0 else table[4]
  print (tn)
  print (fp)
  print (fn)
  print (tp)
...
#' This is the main test harness method.
#' 
#' It's up to e.g. caret whether there's an 
...
evaluate_model <- function(
  ...
  for (i in 1:numFolds) {
    ...
    if (trainSMOTE) {
      #model.fit <- SMOTE(formula, train.data,perc.over = 200, k = 5, perc.under = 200, learner = trainMethod, ...)
      #model.fit <-data =  SMOTE(form = TARGET_FACTOR ~ ., data = data,perc.over = 100, k = 5, perc.under = 200, learner = trainMethod, ...)
      model.fit <-data =  SMOTE(form = TARGET_FACTOR ~ ., data = data,perc.over = 100, k = 5, perc.under = 200)
    }
    else {
