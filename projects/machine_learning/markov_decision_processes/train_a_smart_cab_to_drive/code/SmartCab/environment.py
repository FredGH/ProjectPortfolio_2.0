...
class Environment(object):
    ...
    def reset(self):
        ...
        print "Environment.act() [POST]: Start next trial..."
    ...
    def step(self):
        ...
        if self.primary_agent is not None:
            if self.enforce_deadline and self.agent_states[self.primary_agent]['deadline'] <= 0:
                self.run_cum_reward = self.run_cum_reward + self.cum_reward
                self.done = True
                print "Environment.reset(): Primary agent could not reach destination within deadline!"
                print "Trial: " + self.current_trial
    ...
    def act(self, agent, action):
        ...
        if agent is self.primary_agent:
            ...
            if state['location'] == state['destination']:
                self.reach_dest = self.reach_dest+1 
                if state['deadline'] >= 0:
                    reward += 10  # bonus
            ...
            print "self.reach_dest: " + str(self.reach_dest)
            print "self.current_trial: " + str(self.current_trial)
            print "self.penalty: " + str(self.penalty)
            print "self.num_moves: " + str(self.num_moves)
            print "self.cum_reward: " + str(self.cum_reward)
            print "self.run_cum_reward: " + str(self.run_cum_reward)
            ...
            self.status_text = "state: {}\naction: {} - reward: {} - cum reward: {} - run reward: {} \nsuccess_rate: {} - penalty_ratio: {}\nnum_moves: {} - current_trial: {}".format(agent.get_state(), action, reward, self.cum_reward, self.run_cum_reward, success_rate, penalty_ratio, self.num_moves, self.current_trial)
